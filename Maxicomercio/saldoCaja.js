...
function corteCajaTerminal(){
	...
	*/
	...
	try
	{
		...
		catEgresoCorte=parseInt(eHKCurUser.EGetSetting("Deminus", "Config", "lvar_catEgresoXCorte_tp", "0"));
		if(catEgresoCorte==0){
			catEgresoCorte=parseInt(eHKCurUser.EGetSetting("Maxicomercio", "Config", "lvar_catEgresoXCorte_tp", "0"));
		}		
		catEgreso=parseInt(eHKCurUser.EGetSetting("Deminus", "Config", "lvar_catEgresoXFaltante_tp", "0"));
		if(catEgreso==0){
			catEgreso=parseInt(eHKCurUser.EGetSetting("Maxicomercio", "Config", "lvar_catEgresoXFaltante_tp", "0"));
		}
		catIngreso=parseInt(eHKCurUser.EGetSetting("Deminus", "Config", "lvar_catIngresoXSobrante_tp", "0"));
		if(catIngreso==0){
...
var vT = fastCount.cDetCorte.dbtargetas;
var vD = fastCount.cDetCorte.dbdepositos;
var vC = fastCount.cDetCorte.dbcheques;
var vV = fastCount.cDetCorte.dbvales;
var sumaOtros = vT + vD + vC + vV;
...
if (sumaOtros > 0) {
	...
	if (vT > 0) {
		...
		S = S + Impresora.AligTextInStr("$" + vT, 10, 1, " ");
		Impresora.Texto(S);
	}
	if (vD > 0) {
		S = Impresora.AligTextInStr(" Dep\u00f3sitos", 20, 0, " ");
		S = S + Impresora.AligTextInStr("$" + vD, 10, 1, " ");
		Impresora.Texto(S);
	}
	if (vC > 0) {
		S = Impresora.AligTextInStr(" Cheques", 20, 0, " ");
		S = S + Impresora.AligTextInStr("$" + vC, 10, 1, " ");
		Impresora.Texto(S);
	}
	if (vV > 0) {
		S = Impresora.AligTextInStr(" Vales", 20, 0, " ");
		S = S + Impresora.AligTextInStr("$" + vV, 10, 1, " ");
		Impresora.Texto(S);
	}
