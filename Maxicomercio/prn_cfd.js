...
sql ="SELECT Venta.Fecha, Venta.Referencia, Cliente.Nombre AS NombreCliente, Domicilio.Direccion,Domicilio.Colonia, Ciudad.Nombre AS NombreCiudad, Domicilio.CodPos,EdoProv.Nombre AS Estado,Pais.Nombre AS Pais, Cliente.RFC, Venta.uf_FechaCFD, venta.xmlFormaPago AS fpContraprestacion, venta.xmlCondicionesPago AS condicionPago, venta.xmlMetodoPago AS metodoPago, venta.xmlNumeroCuentaPago AS numCta FROM ((((Venta INNER JOIN Cliente ON Venta.ICliente=Cliente.Sys_PK) INNER JOIN Divisa ON Divisa.Sys_PK = Venta.IDivisa) INNER JOIN (ut_CFD INNER JOIN (ut_CFDFolio INNER JOIN ut_CFDInfo ON ut_CFDFolio.uf_CFDInfo=ut_CFDInfo.Sys_PK) ON ut_CFD.uf_CFDFolio=ut_CFDFolio.Sys_PK) ON Venta.Sys_PK=ut_CFD.uf_IVenta) LEFT JOIN (Domicilio LEFT JOIN (Ciudad LEFT JOIN (EdoProv LEFT JOIN Pais ON EdoProv.IPais=Pais.Sys_PK) ON Ciudad.Estado=EdoProv.Sys_PK) ON Ciudad.Sys_PK = Domicilio.ICiudad) ON Domicilio.Sys_PK = Cliente.Domicilio1) WHERE Venta.Sys_Pk=" + PKFacturaCFD + " AND venta.documento=4;";
...
function ImprimirDetalle(PKFacturaCFD){
	...
	while(!Rst.EOF)
	{
		...
		cStr = Rst("NumDocAduana").Value+"";//Num doc aduama
		cStr = Impresora.AligTextInStr(cStr,5,1," ");
		sDetalle=sDetalle+cStr;
