...
var cmdSQLDetalleCortes="SELECT * FROM qryDetalleCorte WHERE ICorte=@PKCorte";
...
var file; //Permitir datos adjuntos
var saveP;
var Adjuntar;
...
function CrearPanel(){
	...
	Application.UIShortCuts.CreateAction("P_Fin_B05","Egreso por ajuste",0,"","","","UIFinanzas.Retiro",0,"","","",0);
	...
	Application.UIShortCuts.CreateAction("P_Fin_B11","Contabilizar movimiento de cuenta",0,"","","","UIFinanzas.ContabilizarMovCuenta",0,"","","",0);
	...
	//jv - Agregar acción para refoliar
	Application.UIShortCuts.CreateAction("N_Fin_001","Refoliar",0,"","","","UIFinanzas.Refoliar",0,"","","",0);	
	Application.UIShortCuts.CreateAction("N_Fin_002","Archivos adjuntos",0,"","","","UIFinanzas.Adjuntos",0,"","","",0);
	Application.UIShortCuts.CreateAction("N_Fin_003","Archivos adjuntos",0,"","","","UIFinanzas.Adjuntos_",0,"","","",0);		
	
	Application.UIShortCuts.CreateAction("P_Fin_A60","Buscar Cuenta",0,"","","","UIFinanzas.LanzarVerCfdi",0,"","","",0);
	
	//rfl - Formulario de Captura Rápida
	Application.UIShortCuts.CreateAction("P_2014_R2_1","Captura Rápida",0,"","","","UIFinanzas.PolizaFast",0,"","","",0);
	Application.UIShortCuts.CreateAction("P_2014_R2_2","Busqueda",0,"","","","UIFinanzas.BuscaConcepto",0,"","","",0);
	Application.UIShortCuts.CreateAction("P_2014_R2_3","DesAplicar Póliza",0,"","","","UIFinanzas.DesAplicarPolizaEdit",0,"","","",0);
...
if (Brw==null)
	{
		...
		//Brw.Caption="Contabilidad y finanzas";
		Brw.Caption="Cuentas Contables";
...
function NuevaPoliza(){
	...
	if (dlg==null){			
		...
		dlg.CmdKeyDown="UIFinanzas.Command_KeyDown_Pol";
		
		/***************     By JFrank - 08/Sept/09   */
		dlg.AddButton("Guardar y Aplicar (F6)","P_Fin_A04");
		dlg.AddButton("Guardar como Borrador (F7)","P_Fin_A13");
		dlg.AddButton("Guardar como plantilla (F9)","P_Fin_A14");
				//jv - Agregar boton para agregar pólizas
		dlg.AddButton("Archivos adjuntos (F10)", "N_Fin_003");
		dlg.AddButton("Usar plantilla (F11)","P_Fin_A15");
		dlg.AddButton("Cerrar (F12)","P_Fin_A19");
		
		dlg.ShowButtons();
		FormPoliza=dlg.GetAXObject();	
		UIFoliosPolizas.FoliosPolizas.setObject(Application.ADOCnn);
		FormPoliza.SetObjectsFl(UIFoliosPolizas.FoliosPolizas); //jv - Enviar objeto para control de polizas
...
function LimpiarPolizaActual(PKPoliza)
{
	var sClear = "DELETE FROM CDPoliza WHERE FK_CPoliza_Detalle = " + PKPoliza + ";";
	Application.ADOCnn.Execute(sClear);
}
...
function GuardarPoliza(A,bAplicar){
	...
	{
		FormPoliza=dlg.GetAXObject();
		LimpiarPolizaActual(FormPoliza.Poliza.Sys_PK);
		if (FormPoliza.ActualizarPoliza()==true)
		{
			...
			if (bAplicar)
			{
				if(FormPoliza.Poliza.Debe==FormPoliza.Poliza.Haber && FormPoliza.Poliza.Debe !=0 && FormPoliza.Poliza.Haber !=0)
				{
				...
				{
					eBasic.eMsgbox("La póliza se guardó como borrador, porque las sumas de Debe y Haber son diferentes, o no se capturarón las cuentas.",6);
				}
			}
			
			if(Adjuntar)
			{
				Adjuntos(FormPoliza.Poliza.Sys_GUID, FormPoliza.Poliza.Referencia);
			}
			saveP = true;
			ActualizarQPoliza();
			...
			else
			{
				if(!Adjuntar)
				{
					eBasic.eMsgbox("La póliza se guardó correctamente.",6)
						/*
						if(eBasic.eMsgbox("¿Desea crear nueva póliza?",4)==6)
						{
							FormPoliza.PrepareNew();
	...
	if (dlg==null){			
		...
		if (dlg != null){
				...
				if (FormPoliza.Poliza.Aplicada){
					...
					dlg.AddButton("Desaplicar","P_2014_R2_3");
					...
					dlg.AddButton("Usar plantilla (F11)","P_Fin_A15");
					dlg.AddButton("Cerrar (F12)","P_Fin_A19");	
					//dlg.AddButton("Buscar","P_Fin_A60");
					dlg.AddButton("Ver Comprobantes","P_Fin_A60");
				}
				dlg.ShowButtons();
		}		
	}else
...
if (Brw==null)
	{
		...
		/*
		Validar existencia de folios... En caso contrario refoliar...
		*/
		
		R2=Application.adocnn.execute("Select count(sys_pk) as conta from cpoliza");
		...
		var v2 = R2("conta").Value;
		
		if(v2>0)
		{
			R=Application.adocnn.execute("Select count(sys_pk) as conta from foliospolizas");
	
			if(R==null){
				if(eBasic.eMsgbox("Las pólizas no se encuentran foliadas, ¿Desea iniciar el proceso para foliar las pólizas?, esto puede tardar varios minutos.", 4) == 6)
				{	
					Refoliar(false);
				}
			}
		...
		Brw.ReportsFolder=eBasic.AddSlashPath(Application.GetPath(0));//Reportes.CarpetaConta_Finanzas;
		...
		//jv - Agregar boton
		Brw.AddButton("Refoliar","N_Fin_001");
		Brw.AddButton("Archivos adjuntos","N_Fin_002");
		...
		Brw.SetGroup("Tipo");
		file = eBasic.eCreateObject("geDocRep.Manager");
	}
	else
		Brw.Zorder();
...
Brw=Application.Browsers.GetBrowser(BrwName);
	if (Brw!=null){			
		...
		Brw.SetColumnWidth("fe",80);
		Brw.SetColumnWidth("fp",80);
		Brw.SetColumnWidth("fpt",80);	

		Brw.SetCaptionByFieldName("fet","FET");
		Brw.SetCaptionByFieldName("fe","FE");
		Brw.SetCaptionByFieldName("fp","FP");		
		Brw.SetCaptionByFieldName("fpt","FPT");		
...
if (Brw==null)
	{		
		...
		Brw.AddButton("Contabilizar","P_Fin_B11");						
...
if (Brw!=null)
	{			
		...
		Brw.SetColumnWidth("Contabilizado",60);
...
function ActualizarBotones(){
	...
	if (Brw!=null){			
		...
		Btn=Brw.GetButtonByIDAction("P_Fin_B11");
		if(Btn!=null){
			
			if (Brw.GetFieldValue("Contabilizado")==true)					
			{				
				
				Btn.Caption="Quitar Contabilización";
			}
			else
			{
			
				Btn.Caption="Contabilizar";
			}
		}
...
function RedimensionarColDetalleCorte(BrwName){
 ...
 function ProcesarReporte(AXForm,Destino){
	...
	Formato = eBasic.AddSlashPath(Application.SourceAdmin.GetPath(0))+eBasic.AddSlashPath("Reports")+"Contabilidad y finanzas\\Poliza.xpd";
	if(Formato==""){
...
function Command_KeyDown(KeyCode){
	...
	switch(KeyCode){
		case 114: //F3
			id="P_Fin_A17"; break;//Modificar Poliza
		case 117: //F6
			id="P_Fin_A04"; break;
	}
	if (id!=""){ //F6		
...
function Command_KeyDown_Pol(KeyCode){
	if(KeyCode==null) return 0;		
	var id="";
	//Log("Tecla: "+KeyCode);
	switch(KeyCode){
...
/*
		...
		dlg.AddButton("Usar plantilla (F11)","P_Fin_A15");
		dlg.AddButton("Cerrar (F12)","P_Fin_A19");
		dlg.AddButton("Captura Rápida (F8)","P_2014_R2_1");
*/
...
function Refoliar(ask)
{
	...
	UIFoliosPolizas.FoliosPolizas.refoliar(true);
}
...
function Adjuntos(g, referencia)
{
	var guid;
	var ref;
	try
	{
		...
		file.SetADOCnn(Application.AdoCnn);
		file.CanAddDocument = "FX1-10-100-005-A"
		file.CanAddLink = "FX1-10-100-005-B"
		file.CanEdit = "FX1-10-100-005-C"
		file.CanDeleteLink = "FX1-10-100-005-D"
		file.CanDeleteDocument = "FX1-10-100-005-E"
		
		if(g==null || referencia == null)
		{
			var PK;
			var B;
			B=Application.Browsers.GetBrowser("qPoliza");	
			...
			var R;
			var SQL = "SELECT sys_GUID, referencia FROM cPoliza WHERE Sys_PK="+PK;
			R=Application.Database.OpenRecordset(SQL);
			if(R==null){
				Log("Error al obtener datos de la póliza.");
				return 0;
			}
			if(R.BOF && R.EOF){
				Log("Error al obtener datos de la póliza.");
			...
			guid = R.Fields("sys_GUID").Value;	
			ref = R.Fields("referencia").Value;	
		...
		file.BrowseDlg(guid, ref, "cPoliza");
...
if (B!=null){
		...
		if (PKPoliza!=null){
			try
			{	
			var appExe = "r5.vercfdis.porpoliza.exe " + QName + " " + PKPoliza;
...
function Adjuntos_(A)
{
var FormPoliza;	
var dlg;
	if(!saveP)
	{
		if(eBasic.eMsgbox("Aún no ha guardado la póliza, ¿Desea guardarla?",4)==7)
			return -1;
		...
		if(saveP)
		{
			eBasic.eMsgbox("La póliza ha sido guardada en modo borrador.",6);
			var dlg;
			dlg=Application.AXForms.AXForm(A.Context.TagData);
			...
			if (dlg!=null){
				FormPoliza=dlg.GetAXObject();
				FormPoliza.CargarPoliza();
				//dlg.GetButtonByIDAction("P_Fin_A04").Enabled = false;
				//dlg.GetButtonByIDAction("P_Fin_A13").Enabled = false;
			}
		}
		//ActualizarPoliza();
		//AplicarPoliza();
...
dlg=Application.AXForms.AXForm(A.Context.TagData);
	...
	{
		FormPoliza=dlg.GetAXObject();
		FormPoliza.CapturaRapida();
	}
	else
	{
		Log("Error al acceder al formulario.");
	}
}

function BuscaConcepto(A){
var dlg;
var FormPoliza;
dlg=Application.AXForms.AXForm(A.Context.TagData);
	if (dlg!=null)
	{
		FormPoliza=dlg.GetAXObject();
		FormPoliza.AbrirCatalogoConceptos();
	}
	else
	{
		Log("Error al acceder al formulario.");
...
function DesAplicarPolizaEdit(A){
...
dlg=Application.AXForms.AXForm(A.Context.TagData);
	...
	{
		FormPoliza=dlg.GetAXObject();
		if (Contabilidad.BLogic.DesAplicarPoliza(FormPoliza.PKFolioPoliza)){
			eBasic.eMsgbox("La póliza se desaplicó correctamente",6);
			
			Application.ActiveWindow().ExecuteButtonAction("P_Fin_A19");//Cerra Poliza Activa
...
function ShowRelacionaCuentas(){

if(!Application.UIUsers.CheckItem("FX1-71-04-004"))  //PERMITIR ACCESO
	return 0;

var Obj = eBasic.eCreateObject("RelCtaSat.CtaSat");
if(Obj==null){ Log("No se pudo."); return 0; }
Obj.setConexion(Application.adocnn);
Obj.ShowForm();
}


function ContabilizarMovCuenta(A,PK){
var Brw;
var PK;
Brw=null;
	...
	if(Brw.GetButton(1,7).Caption=="Contabilizar"){		
		if (LBEfectivo.ContabilizarMovCuenta(PK)){
			Brw.GetButton(1,7).Caption="Quitar Contabilización";
			Brw.RefreshRst();
		}else
			Log(LBEfectivo.LastErrorDescrip);
	}else{				
		if (LBEfectivo.ContabilizarMovCuenta(PK,false)){			
			Brw.GetButton(1,7).Caption="Contabilizar";
			Brw.RefreshRst();
		}else
			Log(LBEfectivo.LastErrorDescrip);
	}	
